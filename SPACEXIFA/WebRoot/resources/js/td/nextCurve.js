    ...
    if (redata.length === 405 || redata.length % 135 === 0) {
        for (var i = 0; i < redata.length; i += 135) {
            ...
            if (redata.substring(8 + i, 12 + i).toString() == $("#machineid").val()) {
                ...
                //根据工作号id去查询工作号信息
                if (vlog) {
                    var cardid = parseInt(redata.substring(99 + i, 103 + i));//工作号id
                    var junctionId = parseInt(redata.substring(115 + i, 119 + i));//焊缝id
                    $.ajax({
                        type: "post",
                        async: true,
                        url: "td/getWorkInfoByCardId",
                        data: {
                            cardId: cardid,
...
// 				}
                ...
                // 	$("#r6").html(parseInt(redata.substring(91 + i, 95 + i), 10));
                // }
                $("#r4").html(worktime.insName);//班组
                $("#l2").html(worktime.machineno);//焊机型号
                $("#l3").html(redata.substring(8 + i, 12 + i));//采集编号
                for (var k = 0; k < welderName.length; k++) {
                    if (welderName[k].fid == parseInt(redata.substring(0 + i, 4 + i).toString())) {    //焊工号
                        //$("#m3" + machine[f].fid).html(welderName[k].fname);
                        $("#l4").html(welderName[k].fname);//操作人员
                        break;
                    }
                }
                var imgnum = $("#type").val();
